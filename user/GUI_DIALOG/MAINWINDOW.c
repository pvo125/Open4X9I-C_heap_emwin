...
#define ID_BUTTON_BACK	(GUI_ID_USER + 0xE)
#define ID_BUTTON_PEN1	(GUI_ID_USER + 0xF)
#define ID_BUTTON_PEN2	(GUI_ID_USER + 0x10)
#define ID_BUTTON_PEN3	(GUI_ID_USER + 0x11)
...
extern WM_HWIN Settings(void);
extern LCD_LayerPropTypedef          layer_prop[GUI_NUM_LAYERS];


extern volatile uint8_t hwin;	
...
WM_HWIN Window1Layer0(void);
WM_HWIN Window2Layer0(void);
WM_HWIN WindowLayer1(void);
WM_HWIN _Paint(void); 
...
GUI_COLOR window1color;//=GUI_LIGHTBLUE;
GUI_COLOR window2color;//=GUI_DARKCYAN;
uint8_t Layer;
volatile uint8_t change_Layer;
...
*/
#if 0
int _cbButtonPenSize1(const WIDGET_ITEM_DRAW_INFO *pDrawItemInfo){
	
	#define RAD 3
	GUI_RECT Rect;
	switch(pDrawItemInfo->Cmd){
		case /*WIDGET_ITEM_DRAW*/WIDGET_ITEM_DRAW_BACKGROUND:
			GUI_SetPenSize(1);
			...
			GUI_SetPenSize(7);
			GUI_DrawPoint((Rect.x1-Rect.x0)/2,(Rect.y1-Rect.y0)/2);
		break;
		default:
...
int _cbButtonPenSize2(const WIDGET_ITEM_DRAW_INFO *pDrawItemInfo){
...
int _cbButtonPenSize3(const WIDGET_ITEM_DRAW_INFO *pDrawItemInfo){
...
*       _cbPaint
*/
static void _cbPaint(WM_MESSAGE * pMsg) {
  int     NCode;
  int     Id;
			...
			case ID_BUTTON_BACK:
				switch(NCode){
					...
					case WM_NOTIFICATION_RELEASED:
							EXTI->IMR |=EXTI_IMR_MR2|EXTI_IMR_MR10;
							WM_HideWindow(hWin_paint);
							WM_ShowWindow(hWin0);
					break;	
				}
			break;
...
*       _cbWin_0
*/
static void _cbWindow1Layer0(WM_MESSAGE * pMsg) {
  int     NCode;
  int     Id;
...
//#endif
    ...
    break;
      ...
      switch(NCode) {
			case WM_NOTIFICATION_CLICKED:
				hIcon=WM_GetDialogItem(hWin0,ID_ICON_alarma);
				WM_DeleteWindow(hIcon);
				hIcon=ICONVIEW_CreateEx(300,2,34,34,hWin0,WM_CF_SHOW|WM_CF_HASTRANS,0,(GUI_ID_USER + 0x0A),24,24);
    ...
    break;
      ...
      switch(NCode) {
			case WM_NOTIFICATION_CLICKED:
				hIcon=WM_GetDialogItem(hWin0,ID_ICON_alarmb);
				WM_DeleteWindow(hIcon);
				hIcon=ICONVIEW_CreateEx(336,2,34,34,hWin0,WM_CF_SHOW|WM_CF_HASTRANS,0,(GUI_ID_USER + 0x0B),24,24);
				ICONVIEW_SetBkColor(hIcon, ICONVIEW_CI_SEL, GUI_DARKBLUE|0x80000000);	
...
*       _cbWin_1
*/
static void _cbWindow2Layer0(WM_MESSAGE * pMsg) {
  int     NCode;
  int     Id;
  switch (pMsg->MsgId) {
	case WM_PID_STATE_CHANGED:
...
*       _cbLayer_1
*/
static void _cbWindowLayer1(WM_MESSAGE * pMsg) {
	int     NCode;
  int     Id;
...
WM_HWIN Paint(void) {
	...
	ICONVIEW_Handle hIcon;
	uint8_t i;	
	hWin_paint=WM_CreateWindowAsChild(0,0,480,272,WM_GetDesktopWindowEx(0),0, _cbPaint,0);
	hButton=BUTTON_CreateEx(430,240,40,25,hWin_paint,WM_CF_SHOW|WM_CF_HASTRANS,0,ID_BUTTON_BACK);
	BUTTON_SetText(hButton,"BACK");
	hButton=BUTTON_CreateEx(180,5,25,25,hWin_paint,WM_CF_SHOW|WM_CF_HASTRANS,0,ID_BUTTON_PEN1);
	
	hButton=BUTTON_CreateEx(230,5,25,25,hWin_paint,WM_CF_SHOW|WM_CF_HASTRANS,0,ID_BUTTON_PEN2);
	
	hButton=BUTTON_CreateEx(280,5,25,25,hWin_paint,WM_CF_SHOW|WM_CF_HASTRANS,0,ID_BUTTON_PEN3);
	
	for(int i=0;i<9;i++)
	{
		hIcon=ICONVIEW_CreateEx(15,i*30,30,30,hWin_paint,WM_CF_SHOW,0,xD[i],30,30);
		ICONVIEW_SetBkColor(hIcon, ICONVIEW_CI_BK, color[i]);	
	}	
...
WM_HWIN Window1Layer0(void) {
		...
		hWin0=WM_CreateWindowAsChild(0,0,480,272,WM_GetDesktopWindowEx(0),0, _cbWindow1Layer0,0);	
		for(i=0;i<3;i++)
		{
		...
		for(i=0;i<2;i++)
		{
			hIcon=ICONVIEW_CreateEx(300+36*i,2,34,34,hWin0,WM_CF_SHOW|WM_CF_HASTRANS,0,(GUI_ID_USER + 0x0A+i),24,24);
#ifdef CODEFLASH
...
WM_HWIN Window2Layer0(void) {
		...
		hWin1=WM_CreateWindowAsChild(0,0,480,272,WM_GetDesktopWindowEx(0),0, _cbWindow2Layer0,0);	
		for(i=0;i<3;i++)
		{
...
WM_HWIN WindowLayer1(void){
		...
		hWin2=WM_CreateWindowAsChild(0,0,480,272,WM_GetDesktopWindowEx(1),WM_CF_SHOW, _cbWindowLayer1,0);
...
void MainTask(void){
	...
	window1color=RTC->BKP2R;
	window2color=RTC->BKP3R;
